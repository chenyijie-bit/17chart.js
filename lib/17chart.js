;(function (global, factory) {
  ...
  const handler = (option, userOption) => {
    ...
    if (isPercent) {
      ...
      // label的值需要做格式化处理
      option.series.forEach((serieItem) => {
        set(serieItem, 'label.formatter', dataFormatterFn)
      })
      if (sget(option, 'yAxis.minInterval') >= 1) {
    ...
    if (getIsLegendShow(userOption)) {
      const gridBottom = sget(option, 'grid.bottom')
   ...
   * 判断是否有legend
   */
  const getIsLegendShow = (userOption) => {
    let isShow = true
    const nameList = sget(userOption, 'name')
    if (
      sget(userOption, 'legend.show') === false ||
      !nameList ||
      !nameList.length
    ) {
      isShow = false
   ...
   * @returns {number}
   */
  const _getExtraWidth = (name, isPercent, type) => {
  ...
  const merge = (defaultOption, userOption) => {
    ...
    if (is2Array(data)) {
      ...
      // 挂载图表数据
      const seriesData = data.map((array, index) => {
